    ...
    const vec4 pos)
{
    ...
    ray r;
    r.orig = pos.xyz; //NOTE: unoptomized memory access.
...
__kernel void batched_path_tracer(
    lobal unsigned int* out_tex,
    const __global float* ray_buffer,
    const __global material* material_buffer,
    const __global sphere* spheres,
    const __global plane* planes,
//Mesh
    ...
    const unsigned int width,
    const unsigned int height,
    const unsigned int x_batch_offset,
    const unsigned int y_batch_offset,
    const vec4 pos,
    unsigned int magic)
{
    int id = get_global_id(0);
...
//Mesh
    ...
    unsigned int magic)
{
    const vec4 sky = (vec4) (0.02, 0.1, 0.1, 0);
    //return;
    int id = get_global_id(0);
    int x  = id%width;
    int y  = id/width;
    ...
    int seed1 = x+(int)((float)magic*10*sin((float)magic)),
        seed2 = y+(int)((float)magic*10*cos((float)magic));
    ...
    for(int i = 0; i < 126; i++)
    {
        ...
        for(int bounces = 0; bounces < NUM_BOUNCES; bounces++)
        {
            ...
            vec3 sample_dir =  uniformSampleHemisphere(rand1, rand2);
        ...
        fin_colour += clamp(accum_color,0.f,1.f) * (1.f/126.f);
    }

    //   colour = mix(colour, colours[0],  result.mat.reflectivity);

    out_tex[x+y*width] = get_colour((vec4) (fin_colour, 0) );
    ...
    const unsigned int sample)
{
    ...
    float4 temp = mix((float4)(
                      ...
                      (float4)(
                          ...
                          (float)out_tex[offset].w), (float)sample/1.f);
    /*vec4 temp =  (float)(
        (float)fresh_frame_tex[offset].x,
        (float)fresh_frame_tex[offset].y,
        (float)fresh_frame_tex[offset].z,
