...
__kernel void ic_screen_textures(
    __global vec4* pos_tex,
    __global vec4* nrm_tex,
    ...
    const __global vec3* normals)
{
    ...
    ray r;
    r.orig = pos.xyz; //NOTE: unaligned memory access.
    ...
    if(!collide_all(r, &result, material_buffer, spheres, planes, meshes, indices,
                    vertices, normals))
    {
        pos_tex[offset] = result.point;
        nrm_tex[offset] = (vec3)(0,0,0);
        return;
    }

    pos_tex[offset] = result.point;
    nrm_tex[offset] = result.normal;
}
    ...
    unsigned int magic)
{
    ...
    for(int i = 0; i < 24; i++)
    {
        ...
        fin_colour += clamp(accum_color,0.f,1.f) * (1.f/24.f);
    }
